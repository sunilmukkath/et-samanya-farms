...
export default async function LedgersPage({
  ...
  const rows = chronological
    ...
    .reverse();
  const featured = accounts.filter(
    (row) => ["cash", "bank", "upi", "creditors", "debtors"].includes(row.id) || row.type === "expense" || row.type === "income",
  );

  return (
    <div className="mx-auto max-w-xl px-4 py-5">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-clay">Ledgers</p>
      <h1 className="font-display text-3xl sm:text-4xl">{current?.name ?? "Ledger"}</h1>
            ...
            <li key={line.id} className="px-4 py-3 text-sm">
              <div className="flex justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="truncate">{voucher?.partyName || voucher?.narration || "Entry"}</p>
                </div>

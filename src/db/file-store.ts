...
import { chartOfAccounts, newSecretToken } from "@/lib/accounts";
...
function seedBookSettings(now: Date): BookSettingsRow {
  return {
    id: "farm",
    gstin: null,
    pan: null,
    smsToken: newSecretToken(),
    createdAt: now,
    updatedAt: now,
  };
}
...
function revive(data: Partial<FarmFile>): FarmFile {
  ...
  return {
    ...
    bookAccounts: data.bookAccounts?.length
      ? data.bookAccounts.map((row) => ({ ...row, createdAt: new Date(row.createdAt) }))
      : seedAccounts(now),
...
function seed(): FarmFile {
  ...
  return {
    ...
    bookVouchers: [],
    bookLines: [],
    bookSettings: seedBookSettings(now),
  };
}
...
export const fileStore = {
  ...
  saveBookSettings(patch: Partial<BookSettingsRow>) {
    return mutate((data) => {
      const now = new Date();
      data.bookSettings = {
        ...(data.bookSettings ?? seedBookSettings(now)),
        ...patch,
        updatedAt: now,
      };
      return data.bookSettings;
